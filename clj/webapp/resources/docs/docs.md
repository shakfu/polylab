...
The `home-routes` handler in the `webapp.routes.home` namespace
...
The `home-page` function will in turn call the `webapp.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `webapp.handler` namespace:
...
Request middleware functions are located under the `webapp.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `webapp.dev-middleware` namespace found in
