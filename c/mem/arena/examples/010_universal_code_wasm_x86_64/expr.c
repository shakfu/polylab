...
// TODO: consider moving setting all these parameters on defined(__wasm__) to
// arena.h 'cause I think it's general useful for arena.h to just do all of
// this automatically for you.
#define ARENA_BACKEND ARENA_BACKEND_WASM_HEAPBASE
#define ARENA_NOSTDIO
// We are using __builtin_trap() because we are assuming only clang can compile
// to wasm as of today
#define ARENA_ASSERT(cond)                                                    \
    (!(cond)          ? printf("%s:%d: %s: Assertion `%s' failed.", __FILE__, \
...
// Implementing or own is_digit and is_print, because ctype is not available on
// wasm32 in clang for some reason
...
Node* parse_binop(Lexer* lexer)
{
    ...
    if (node->binop.rhs == NULL)
        return NULL;
    return node;
}

Node* parse_expr(Lexer* lexer) { return parse_binop(lexer); }
...
int main(void)
{
    ...
    // Just making several wasm32 page size allocations to test the wasm32
    // memory grow. x86_64 should not care because it's not a toy platform
    // unlike wasm.
    ...
    printf("  Default region size: %d words (%zu bytes)\n",
           ...
           ARENA_REGION_DEFAULT_CAPACITY * sizeof(uintptr_t));
    printf("  Regions (%zu):\n", count_regions(nodes));
    size_t n = 0;
    for (Region* iter = nodes.begin; iter != NULL; iter = iter->next) {
        printf("    Region %zu: address = %p, capacity = %zu words (%zu "
               "bytes), count = %zu words (%zu bytes)\n",
               n, iter, iter->capacity, iter->capacity * sizeof(uintptr_t),
