    ...
    if (olen >= b->mlen) {
        ...
        reallocStrategy:;
            ...
            if (x == NULL) {
                ...
                if (NULL
                    == (x = (unsigned char*)bstr__realloc(
                            b->data, (size_t)(len = olen)))) {
                    return BSTR_ERR;
                }
            }
        } else {
...
int binsert(bstring b1, int pos, const_bstring b2, unsigned char fill)
{
    ...
    if (pos < 0 || b1 == NULL || b2 == NULL || b1->slen < 0 || b2->slen < 0
        || b1->mlen < b1->slen || b1->mlen <= 0)
        return BSTR_ERR;
             ...
             unsigned char fill)
{
    ...
    if (pos < 0 || len < 0 || (pl = pos + len) < 0 || b1 == NULL || b2 == NULL
        || b1->data == NULL || b2->data == NULL || b1->slen < 0 || b2->slen < 0
        || b1->mlen < b1->slen || b1->mlen <= 0)
        return BSTR_ERR;
                             ...
                             int pos, instr_fnptr instr)
{
    ...
    int* d;
    int static_d[INITIAL_STATIC_FIND_INDEX_COUNT
                 + 1]; /* This +1 is unnecessary, but it shuts up LINT. */
    ptrdiff_t pd;
    ...
    if (b == NULL || b->data == NULL || find == NULL || find->data == NULL
        ...
        || b->mlen < 0 || b->slen > b->mlen || b->mlen <= 0 || b->slen < 0
        || repl->slen < 0)
        return BSTR_ERR;
    ...
    */
    ...
    if (BSTR_OK == (ret = balloc(b, b->slen + acc + 1))) {
        ...
        for (i = slen - 1; i >= 0; i--) {
            ...
            if (auxr->slen) {
                bstr__memmove(b->data + s + acc - auxr->slen, auxr->data,
                              auxr->slen);
            }
            acc += delta;
        }
...
/*  int bfindreplacecaseless (bstring b, const_bstring find, const_bstring
 * repl, int pos)
 *
...
{
    struct bstrList* sl = (struct bstrList*)bstr__alloc(
        sizeof(struct bstrList));
    if (sl) {
