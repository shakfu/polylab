...
General algorithm:
    ...
    a task is enqueued (into the pool) when it can safely be execuated
...
Two Key moments
    ...
    - before the task is started (responsibility of caller)
    - after a task is finished (responsibility of callee)
