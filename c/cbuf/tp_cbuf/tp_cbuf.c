...
inline int cbuf_fillcount(cbuf_record *record) {
    return record->fill_count;
}
...
inline int cbuf_space_contiguous(cbuf_record *record) {
    return min(record->length-record->fill_count, record->length-record->head);
}

inline int cbuf_head(cbuf_record *record) {
    return record->head;
}

inline int cbuf_tail(cbuf_record *record) {
    return record->tail;
}
...
inline int cbuf_produce_bytes(cbuf_record *record, void* dst, const void* src, int count, int len) {
    int copied = 0;
    while ( count > 0 ) {
...
inline int cbuf_set_length(cbuf_record *record) {
