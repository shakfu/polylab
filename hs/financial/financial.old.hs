...
-- | A Cashflow is a flow of cash that may be negative or positive
type Cashflow = Double

-- | Capital is the one thing we all need to survive
type Capital = Double
type Residual = Double
...
-}
pmt :: Capital -> Residual -> Interest -> Periods -> Value
...
-}
lease :: Capital -> Residual -> Interest -> Periods -> Value
...
-}
wacc :: Rate -> Rate -> Debt -> Equity -> Capital -> Value
...
-}
annuity :: Cashflow -> Periods -> Interest -> Value
...
-}
fv :: Value -> Interest -> Periods -> Value
fv pv i n = pv * (1 + i)**n
...
{-|
    ...
    dpv fv i n = fv / (1 + i)**n 
-}
dpv :: [Cashflow] -> Interest -> Periods -> Value
